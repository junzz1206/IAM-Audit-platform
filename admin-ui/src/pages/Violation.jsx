              ...
              filteredRows.map((row, index) => {
                ...
                return (
                  <TableRow key={index} hover sx={{ bgcolor: rowBgColor }}>
                      ...
                      {/* 🌟 1. 카드 정보 칸 (여기가 비어있어서 당겨졌던 거예요!) */}
                      <TableCell>
                        <Box display="flex" alignItems="center" gap={0.5} sx={{ color: '#555' }}>
                          <CreditCardIcon fontSize="small" />
                          <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                            {/* row.card_id가 있으면 뒷번호를 보여주고, 없으면 '정보 없음' 표시 */}
                            {row.card_id ? `****-${row.card_id.slice(-4)}` : '정보 없음'}
                          </Typography>
                        </Box>
                      </TableCell>

                      {/* 🌟 2. 가맹점 칸 (정확한 이름표 매칭) */}
                      <TableCell>{row.merchant_name || row.merchant || '-'}</TableCell>
